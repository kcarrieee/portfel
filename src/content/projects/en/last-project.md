...
The most common mistake, to my mind, is starting with buttons. The system has four groups of variables: Palette, Colors, Spacing, Corner radiuses. Every token is defined twice over — once for PWA Day, once for PWA Night. There is no such thing as "the dark version of the mockup" in our files. If a designer built the screen on tokens, dark mode already exists.
...
Same logic in the radii. But here I went further and locked the radius to the height of the component.
...
- Theming — how the component looks in dark
- A11y card — accessibility attributes

The Datepicker is the clearest example.
...
- if the user didn't enter kopecks, we don't show them on blur: `200,00 → 200`
- if they entered one decimal place, we pad it: `200,2 → 200,20`
- you can type a period or a comma; a period always becomes a comma
- thousands are spaced automatically
- digits only
- the currency symbol appears as you type

3. Took it all the way to code.** An engineer and I built the formatting presets together — an `Intl.NumberFormat` config for each number type. And this, to my mind, is the key moment of the whole case.

4. Documented it.** A guide in the design system: preset tables and before/after examples.
